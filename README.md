...
商品詳細では、公式情報をもとにした注目成分の役割、合わない可能性を切り分ける観察点、手持ちとの違い、変化を見る時期、楽天市場と@cosmeの確認導線を表示します。主要20商品の画像URLはメーカー／ブランド公式配信元に固定し、提案カードでは製品説明のすぐ下に使用前の注意事項を添えます。画像の取得に失敗しても、会話や提案自体は止まりません。口コミ本文は転載しません。

5人それぞれに「すぐ回答／自然な相談／じっくり分析」の返答モードを持ち、OrcaRouterの推論深度と回答テンポを連動させます。相談ログにはアプリ側の保存期限と削除UIを設けず、D1上の既存メッセージと追記内容をマージして全履歴を残します。マイアイテムは公式商品の選択に加えて、ユーザー自身の商品名・カテゴリ・メモも登録できます。
